...
- **`<sl-select>` initial value is a timing minefield.** Three flavours of failure: (a) Alpine's `:value=` lands before Shoelace upgrades and the value is lost; (b) `x-init` + `$nextTick` lands before dynamically-loaded options exist (track sub-picker, where `loadTracks` hasn't returned yet); (c) `.value` set while the target option is still `disabled` (alarm-tone option waiting on library load) is silently ignored. The robust pattern is the `syncSelect($el, want, ...deps)` helper in `events.html`: an `x-effect` that awaits `$el.updateComplete` (Shoelace/Lit render promise) before assigning, and that re-runs whenever the value or any dependent state changes.
